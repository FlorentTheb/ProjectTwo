...
public class ResourcesTest {
    ...
    public ResourcesTest(String testCaseName) {
	...
	public static Collection<String[]> getTestCases() throws XmlException, IOException, SoapUIException {
		...
		WsdlProject project = new WsdlProject("src/test/resources/LibrePlanProject2.xml");
    ...
    public static boolean runSoapUITestCase(String testCase) throws XmlException, IOException, SoapUIException {
		...
		WsdlProject soapuiProject = new WsdlProject("src/test/resources/LibrePlanProject2.xml");
		...
		for (TestSuite suite : testSuites) {
			...
			if (soapuiTestCase == null) {
				...
				if (exitValue == TestRunner.Status.FINISHED) {
					System.out.println("[END] : Cas de test soapUI termine ('" + suite.getName().toUpperCase() + "':["
							+ testCase + "]) : " + exitValue);
					return true;
				}
			}
